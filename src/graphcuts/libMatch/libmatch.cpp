...
/// Read file.
bool loadMatch(const char* nameFile, std::vector<LibMatch>& match) {
    match.clear();
...
/// Write file.
bool saveMatch(const char* nameFile, const std::vector<LibMatch>& match) {
    std::ofstream f(nameFile);
    if( f.is_open() ) {
        std::vector<LibMatch>::const_iterator it = match.begin();
        for(; it != match.end(); ++it)
            f << it->x1 << " " << it->y1 << " "
