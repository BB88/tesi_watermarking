...
namespace stereo_watermarking{
    ...
    void showGraph( float* sim_values,int number_of_marks );
    ...
    float similarity_measures(double* wat, double* retrieve_wat, int coeff_num, std::string first_element, std::string second_element);
    ...
    double* compute_coeff_function(unsigned char* image, int dim, string file_name);
    void compute_magnitude_phase(unsigned char* image, int dim, string file_name );
