...
/// to the value of the closest image pixel. The temp array \a buffer holds a
/// mirrored row.
...
{
    const int dim = std::max(image.w,image.h);
    // The Gaussian kernel is truncated at GaussTruncate sigmas from center
    int ksize = (int)(2.0 * GaussTruncate * sigma + 1.0);
    ksize = ksize > 3 ? ksize : 3;
    if(ksize >= dim) {
        std::cerr << "gauss_convol WARNING. Gaussian kernel must be truncated. "
                  << "Variance too large compared to image size." << std::endl;
        ksize = dim;
    }
    if (ksize % 2 == 0) // Make kernel size odd
        ksize++;
    flnum* kernel = new flnum[ksize+1];

    // Fill in kernel values
    flnum sum = 0;
    for (int i = 0; i <= ksize; i++) {
        flnum x = static_cast<flnum>(i - ksize/2);
        kernel[i] = exp(- x * x / (2 * sigma * sigma));
        sum += kernel[i];
    }
    // Normalize kernel values to sum to 1
    for (int i = 0; i < ksize; i++)
        kernel[i] /= sum;

    flnum* buffer = new flnum[ksize+dim-1];
    ConvHorizontal(image, kernel, ksize, buffer);
    ConvVertical(image, kernel, ksize, buffer);
    delete [] buffer;
    delete [] kernel;
} 
