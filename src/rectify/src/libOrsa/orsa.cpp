...
static int FindCubicRoots(float coeff[4], float x[3])
{
    ...
    if (d >= 0) { // 3 real roots
        ...
        double sqrtQ = sqrt(Q);
        x[0] = static_cast<float>(-2*sqrtQ*cos( theta          /3) - a1/3);
        x[1] = static_cast<float>(-2*sqrtQ*cos((theta + 2*M_PI)/3) - a1/3);
        x[2] = static_cast<float>(-2*sqrtQ*cos((theta + 4*M_PI)/3) - a1/3);
        return 3;
    } else { // 1 real root
...
orsa(const std::vector<imgMatch>& match,
