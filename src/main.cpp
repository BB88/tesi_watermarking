...
*//*     imshow("mark_warped", mark_warped);
    ...
    for (int i=0;i<rect_dim;i++){
        double sum = (double)right_uchar[i] + warped_mark[i];
        if (sum>255){
            sum = 255.0;
        }
        else if (sum<0){
...
*//*    imshow("right_marked", right_marked);
    ...
    bool wat2 = image_watermarking.extractWatermark(marked_right, 256, 256);
    cout<<wat2<<endl;*/
...
//    waitKey(0);
    ...
    /*
    stereo_watermarking::dft_comparison(left_squared.data,squared_right,256,"sinistra","destra");
...
//    cv::Mat disp = imread("/home/bene/ClionProjects/tesi_watermarking/img/gt_disp.png", CV_LOAD_IMAGE_GRAYSCALE);
//    unsigned char *disp_uchar = disp.data;
//    cv::Mat occlusion = imread("/home/bene/ClionProjects/tesi_watermarking/img/occ_map.png", CV_LOAD_IMAGE_GRAYSCALE);
//    unsigned char *occ_uchar = occlusion.data;
//    int rect_dim = 480*640*3;
